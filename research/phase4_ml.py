...
plots_dir = project_root / 'plots' / 'Phase4'
...
# ─────────────────────────── 1. Load & Prepare Data ──────────────────────────
df = pd.read_csv(project_root / 'data' / 'ldi_dataset.csv')
...
plt.tight_layout()
plt.savefig(plots_dir / '1_confusion_matrices.png', **SAVE_KW)
plt.close()
...
plt.tight_layout()
plt.savefig(plots_dir / '2_model_comparison.png', **SAVE_KW)
plt.close()
...
plt.legend()
plt.tight_layout()
plt.savefig(plots_dir / '3_district_error.png', **SAVE_KW)
plt.close()
...
joblib.dump(best_model_obj, models_dir / f'best_model_{best_name.replace(" ","_")}.pkl')
...
df_preds.to_csv(models_dir / 'best_model_predictions.csv', index=False)
